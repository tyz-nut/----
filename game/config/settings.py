...
COLOR_FISH = (108, 196, 208)           # 钩空之后放出来那条鱼。偏青，和钩锁的
                                       # 米色、渔夫的队色都能分开
COLOR_FISH_EYE = (28, 44, 52)          # 鱼眼。有它才有"朝哪边游"的朝向感
FISH_EYE_RATIO = 0.22                  # 鱼眼画多大、离中心多远，都按体型取比例，
                                       # 这样大鱼小鱼长一个样，不用另配一套数
FISH_FIN_RATIO = 0.55                  # 尾巴画多长（相对体型）
...
BLINK_SWING_SPEED = 16.0               # 挥砍动画每游戏秒转多少弧度。纯演出。
# 幻影刺客挥砍期间速度被摁成 0，所以"这一帧结束时它身上有多少速度"就等于
# "有没有人把它推走"。低于这个数的不算被打飞——那是浮点零头和分离那一推的
# 残渣，不是真挨了一下（见 Match.update_blinks）
BLINK_KNOCKBACK_EPSILON = 20.0
                                       # 只影响"挥得多快"，不影响伤害和持续
                                       # 时间（伤害是按秒结算的），所以调它
                                       # 就是纯粹的打击感
PHANTOM_SLASH_SPREAD = 1.1             # 一刀扫过的半张角（弧度）。约 63°
PHANTOM_SLASH_WIDTH = 4                # 弧光粗细
PHANTOM_TRAIL_COUNT = 3                # 残影画几道。0 就是不画
PHANTOM_TRAIL_LAG = 0.045              # 每道残影比前一道落后几秒（走的是挥砍的
                                       # 时间轴，不是真实时间）
PHANTOM_TRAIL_FADE = 0.72              # 最老那道残影淡到几成。1 就是和本体一样亮
PHANTOM_RING_PADDING = 7               # 闪现那个圈比球面大多少

# 分裂（见 characters/splitter.py 和 Match.split_ball）
# 这三个数管的是"裂开那一下摆得开不开"，不是数值强弱——伤害、尺寸、几代都在
# roster 里。它们要一起看，理由写在下面两条里
SPLIT_SPREAD_ANGLE = 0.9               # 两个半身各偏开多少弧度（约 52°）。两半
                                       # 是绕着"远离来袭者"那个方向左右分叉的，
                                       # 所以合起来是 103° 的一把扇子。太小的话
                                       # 两个半身摆不开、当场互相贴住
SPLIT_SPAWN_GAP = 12.0                 # 半身摆开时在两倍半径之外再留这么多空隙。
                                       # 它必须 > 0：碰撞结算就发生在裂开这一帧，
                                       # 两个半身要是还贴着敌人，下一帧各自又算一次
                                       # "刚撞上"，一发撞击能连打出三笔伤害
FUSE_LOCK_SECONDS = 1.0                # 刚分开（或刚合上）的半身这几秒里合不回去。
                                       # 少了它，分裂的那一帧两半本来就贴在相切的位置
                                       # 上，下一帧当场融回原样，分裂等于没发生
...
# 毒刺。绿色系——场上还没有绿色的东西（激光红、刀/锤冷白、蛛丝灰白、穿刺青蓝、
# 幻影紫、吸住品红、大锤木色）。中毒时飘的红字是持续伤害那一套，和它不冲突：
# 那些字是"在掉血"，绿色是"身上有毒"，两件事
COLOR_VENOM = (128, 206, 84)           # 墙上的刺。刺身
COLOR_VENOM_CORE = (206, 248, 158)     # 刺尖那一道高光。刺很小，靠它在一堆
                                       # 墙上的东西里认得出来
COLOR_VENOM_RING = (128, 206, 84)      # 中毒的球外面套的那一圈
COLOR_SPIKE_SPENT = (74, 96, 62)       # 刚扎完、还在蓄力的刺。压暗一档，一眼
                                       # 能看出"现在站过去不扎人"
VENOM_SPIKE_WIDTH = 7                  # 刺根部多宽（像素）。它比蛛丝锚点大得多
                                       # ——刺是要扎人的，得看得见
VENOM_STACK_RING_PADDING = 5           # 中毒那圈离球面多远
VENOM_STACK_RING_MAX = 3               # 圈最多按几层变粗。再多就一样粗了：
                                       # 一直长下去叠到十几层会糊掉半个战场

# ---------------- 望的棋盘 ----------------
# 棋盘把战场划成 GO_BOARD_SIZE × GO_BOARD_SIZE 的方格。**这个数是可以随便调的**
# （用户指定的）：格子边长 = ARENA_SIZE / GO_BOARD_SIZE，所以调大格子变小、
# 棋子的判定跟着变细，而棋盘永远和战场严丝合缝。
#
# 600 除不尽 9（66.67），最后一行/列会窄那么不到一个像素。棋盘是画在战场内的，
# 那一列照样在界内、判定也照样按同一个 cell 算，看不出来。
#
# 注意它和 BALL_SPAWN_MARGIN 的关系：格子边长要明显大于球的直径（默认 44），
# 否则一颗球会同时压着两格，"踩在哪一格"就变成看球心那点浮点误差了。
GO_BOARD_SIZE = 9

COLOR_BOARD_LINE = (50, 58, 76)        # 棋盘的格线。压得很暗——它是地面上的刻度，
                                       # 只是让人看出棋子占的是哪一格，不该和场上
                                       # 那些发光的东西（激光红、刀冷白）抢注意力
BOARD_LINE_WIDTH = 1

# 棋子。战场上本来就是深色的，所以黑子光靠填充色看不见，得描一圈亮边才有个形；
# 白子反过来，描一圈暗边免得糊在亮处
COLOR_STONE_BLACK = (30, 34, 44)
COLOR_STONE_BLACK_EDGE = (152, 164, 188)
COLOR_STONE_WHITE = (238, 242, 250)
COLOR_STONE_WHITE_EDGE = (92, 102, 124)
STONE_EDGE_WIDTH = 2
STONE_RADIUS_RATIO = 0.32              # 棋子半径 = 格子边长 × 这个数。留出空隙，
                                       # 一眼能看出它落在哪一格、旁边那格是空的

# 被棋子炸减速的球，外面套的那一圈。颜色取冷蓝，和中毒那圈（毒绿）分开，
# 也和两位玩家的队色（蓝、橙）都不同——它讲的不是"谁"，是"现在很慢"
COLOR_SLOW_RING = (142, 178, 216)
SLOW_RING_PADDING = 11                 # 比中毒那圈更靠外（那个是 5）。同一个球
                                       # 可能既中着毒又被减速，两圈得能同时看见

# 骑士头顶那条小血条（见 app.draw_knight_hp）。骑士**只有被打死才消失**，
# 所以"还剩多少"是场上最要紧的一个数——满血 30 的球看着和快没血的一模一样，
# 没有这条就完全判断不出该不该让它们去换。画在小球正上方，比球还窄一点
KNIGHT_HP_BAR_WIDTH = 24               # 血条多宽（像素）。骑士半径是 12，所以
                                       # 比球略窄，不会盖到旁边那颗身上
KNIGHT_HP_BAR_HEIGHT = 4               # 血条多厚
KNIGHT_HP_BAR_GAP = 5                  # 血条底边离球顶多远
